...
//#include <time.h> //moi removed since it's not included in the wincedevkit, replacement below
#include "shake.h"
#include "decals.h"
#include "player.h"
#include "weapons.h"
...
//moi, time.h replacement taken from:
//http://www.pocketpcjunkies.com/Uwe/Forum.aspx/wince-vc/4091/Library-for-time-h-Functions
...
BOOL UTIL_TeamsMatch( const char *pTeamName1, const char *pTeamName2 )
{
	...
	{
		if ( !strcmp( pTeamName1, pTeamName2 ) )	// Same Team?
			return TRUE;
	}
...
void EntvarsKeyvalue( entvars_t *pev, KeyValueData *pkvd )
{
	...
	for ( i = 0; i < ENTVARS_COUNT; i++ )
	{
		...
		if ( !strcmp( pField->fieldName, pkvd->szKeyName ) )
		{
...
int CRestore::ReadField( void *pBaseData, TYPEDESCRIPTION *pFields, int fieldCount, int startField, int size, char *pName, void *pData )
{
	...
	for ( i = 0; i < fieldCount; i++ )
	{
		...
		if ( !strcmp( pTest->fieldName, pName ) )
		{
